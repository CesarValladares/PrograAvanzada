...
void imageOperations(char * in_filename, char * out_filename, int doNegative, int rotationAngle, int newScale);
...
void parseOptions(int argc, char * argv[])
{
    ...
    while ( (opt = getopt(argc, argv, "i:o:nr:s:")) != -1 )
    {
        switch (opt)
        {
            case 'i':
    ...
    if (input_ok && output_ok)
    {
        ...
        imageOperations(in_filename, out_filename, doNegative, rotationAngle, newScale);
    }
    else
    {
        usage(argv[0]);
